 ...
 * Set Const Variables
 */
// console.log('Variables', define.ns, global)
...
export default class TaskMaster {
  ...
  setTask() {
    ...
    _.forEach(this.task.types, (type, i) => {
      ...
      gulp.task(`${this.task.name}${define.task.separator}${type}`, (done) => {
        this[type](gulp.src(mergeSrc, {allowEmpty: true,encoding: false}), done);
      });
    });
  }
  ...
  setDeleteWatcher(watcher, conf) {
    ...
    watcher.on('unlink', (filepath) => {
      ...
      if (plugins.util.isFileExists(destFilePath)) {
        deleteSync(destFilePath, {force: true});
        ...
        fs.readdir(destDirPath, function(err, files) {
          if (err) {
            ...
            if (!files.length) {
              deleteSync(destDirPath, {force: true});
   ...
   */
  async setCurrentData(filepath, taskData) {
    const time = new Date().getTime() // NOTE: no cahce
    const metaImport = await import(`../../${define.path.pageConfig}?${time}`);
    const meta = metaImport.default
